    ...
    app.get('/orders/pharmacy/:pharmacyID', (req, res) => {
        ...
        pool.getConnection(function (err, connection) {
            if (err) {
                ...
                // if there is no issue obtaining a connection, execute query and release connection
                connection.query('SELECT * FROM `rapidrx`.`orders` AS o JOIN `rapidrx`.`orderDetails` AS od ON o.orderID = od.orderID WHERE o.pharmacyID = ?', [pharmacyID], function (err, rows, fields) {
                    // if there is an error with the query, release the connection instance and log the error
                    connection.release()
                    if (err) {
